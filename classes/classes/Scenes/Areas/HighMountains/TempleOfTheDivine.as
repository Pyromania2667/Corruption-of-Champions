	...
	public class TempleOfTheDivine extends BaseContent
	{
		...
		public function currentStateOfTemple():void {
			...
			if (flags[kFLAGS.TEMPLE_OF_THE_DIVINE_PROGRESS] >= 7) {
				...
				if (player.isGargoyle()) outputText(" Your pedestal stands right next to Sapphire's, allowing you to keep close to her when you need to rest (using your pedestal allows you to sleep with Sapphire if your relation is high enough).");
		...
		public function SapphireTripleTailOuroboros():void {
			clearOutput();
			spriteSelect(SpriteDb.s_gargoyle);
			outputText("You think it over then remember about " + flags[kFLAGS.ONYX_NAME] + " being there too. The third gargoyle must be starving for sex. Unable to decide which one you want to fuck or get fucked by, you propose the lot of you tail fuck each other, something to which your two stony mates are more then happy to agree. The three of you huddle together, your tails tenderly caressing one another.\n\n");
			outputText("You begin by kissing " + flags[kFLAGS.ONYX_NAME] + " then exchanging with Sapphire, the tree of you taking turns in order to feel each other's properly. You are not surprised to discover that out of the three of you " + flags[kFLAGS.ONYX_NAME] + " is the best kisser. Once you all are suitably aroused you playfully insert your tail in Sapphire’s pussy as she position hers over ");
			if (flags[kFLAGS.ONYX_GENDER] == 1 || flags[kFLAGS.ONYX_GENDER] == 2) outputText("Krystal’s waiting cunt");
			else outputText("Onyx’s rock hard length");
			...
			if (flags[kFLAGS.ONYX_GENDER] == 1 || flags[kFLAGS.ONYX_GENDER] == 2) outputText(". Krystal giggles");
			else outputText(". Onyx chuckles");
			outputText(" in the anticipation of ");
